...
def calculate_metrics(financial_details):
    ...
    annual_debt_service = monthly_payment * 12
    maintenance_cost = rent * (maintenance_perc / 100)
    capex_cost = rent * (capex_perc / 100)
    mgmt_cost = rent * (mgmt_perc / 100)
    operating_expenses = taxes + insurance + utilities + maintenance_cost + capex_cost + mgmt_cost
    effective_gross_income = rent * (1 - vacancy_perc / 100)
    noi = effective_gross_income - operating_expenses
    cash_flow = noi - annual_debt_service

    cap_rate = (noi / price) * 100
    cash_on_cash = (cash_flow / (down + closing + rehab)) * 100
